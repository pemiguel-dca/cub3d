...
void	dda_algo(t_raycaster *rc, t_dda *dda, const char **map)
{
	...
	while (!dda->hit_wall)
	{
		...
		if (map[(int)rc->curr_ray_square.x][(int)rc->curr_ray_square.y] == '1')
			dda->hit_wall = true;
	}
}
...
void __debug_vector(const t_vec2 *vec)
{
...
void	start(t_game *game)
{
	...
	while (stripe < WIDTH)/*For every vertical stripe of the screen*/
	{
		...
		rc = init_rc(game->map, stripe);
		dda.hit_wall = false;
		dda.length_to_next_step = write_vector(abs(1 / rc.ray_dir.x), abs(1 / rc.ray_dir.y));
		...
		// dda_algo(&rc, &dda, game->map);
		// draw_prop = get_draw_properties(&rc, &dda);
		// // mlx_put_image_to_window(game.mlx, game.win, game.sprites.east, 0, 0);
		// printf("%d %d %d\n", draw_prop->draw_end, draw_prop->draw_start, draw_prop->draw_end);
		// // draw(draw_prop);//this function is supost to do the drawing
		// free(draw_prop);
		stripe += 1;
	}
}
